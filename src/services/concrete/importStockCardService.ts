  ...
  try {
    ...
    for (const [index, row] of data.entries()) {
      try {
        ...
        const brandID = stockCard.brandName
          ? await prisma.brand.findUnique({
              where: { brandName: stockCard.brandName },
            })
          : null;
        ...
        const stockCardData = {
          ...
          riskQuantities: stockCard.riskQuantities
            ? Number(stockCard.riskQuantities)
            : null,
        ...
        if (stockCard.companyCode) {
          ...
          if (!company) {
            throw new Error(
              `Company with code ${stockCard.companyCode} not found`
            );
          }
        }
        ...
        // Kategori İşleme
        const categories =
          stockCard.categories?.split(",").map((name) => name.trim()) || [];
        const categoryRecords = await Promise.all(
          categories.map(async (categoryName) => {
            const category = await prisma.stockCardCategory.upsert({
              ...
              create: {
                categoryName,
                categoryCode: categoryName.toLowerCase().replace(/\s+/g, "_"),
              },
            });
        ...
        // Özellik İşleme
        const attributes =
          stockCard.attributes?.split(",").map((attr) => {
        ...
        const prices = [...priceString.matchAll(pricePattern)].map((match) => {
          ...
          if (isNaN(price)) {
            throw new Error(
              `Invalid price value for ${priceListName}: ${match[2]}`
            );
          }
        ...
        // Barkod İşleme
        const barcodes =
          stockCard.barcodes?.split(",").map((barcode) => barcode.trim()) || [];
        ...
        const marketNames =
          stockCard.marketNames?.split(",").map((name) => name.trim()) || [];
