...
export const AttributeController = {
  ...
  createManyAttributes: async (ctx: Context) => {
    const attributesData: StockCardAttribute[] =
      ctx.body as StockCardAttribute[];
    console.log(attributesData);
    try {
      const attributes = await attributeService.createManyAttribute(
        attributesData
      );
      ctx.set.status = 200;
      return attributes;
      ...
      ctx.set.status = 500;
      return {
        error: "Error creating multiple attributes",
  ...
  updateAttribute: async (ctx: Context) => {
    const { id } = ctx.params;
    const attributeData: Partial<StockCardAttribute> =
      ctx.body as Partial<StockCardAttribute>;
    try {
